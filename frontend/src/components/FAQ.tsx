...
export default function FAQ() {
  ...
  const faqData = [
    ...
    },
    {
      ...
      answer:
        "The limit is 3 people per team. You are also welcome to compete solo.",
    },
    {
      question: "Is this open to beginners?",
      answer:
        "Yes. The challenge is open to all skill levels and is a great way to practice problem-solving.",
    },
    {
      question: "What programming languages can I use?",
      answer:
        "You can use any programming language you prefer. You wil not submit code, but rather the final answer to the problem.",
    },
  ];
    ...
    <section className="flex flex-col gap-12 md:gap-20 w-full items-center px-4">
      ...
      <div className="z-20 w-8/9 max-w-5xl flex flex-col gap-8">
        {faqData.map((item, index) => (
          <div
            key={index}
            className="w-full outline-white outline-8 rounded-4xl bg-transparent"
          >
            <button
            ...
            >
              ...
              <img
                ...
                alt="flower"
                className={`w-10 sm:w-14 md:w-16 p-2 shrink-0 transition-transform duration-300 ease-in-out ${
                  activeIndex === index ? "rotate-70" : "rotate-0"
                }`}
              />
            </button>
